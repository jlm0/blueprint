...
- Use `blueprint query --project <this-folder> --type show --boundary <kind:id>` and related query modes to inspect one boundary. Use `blueprint extract --project <this-folder> --boundary <kind:id>` for handoff packets.
...
- Raw HTML, CSS, JavaScript, screenshots, and rendered canvas inspection are fallback/debug evidence. Do not use them as the primary source when a structured sidecar field or CLI packet can answer the question.
