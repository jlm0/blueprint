...
The Primitives board renders token groups, primitive families, primitive state sets, and state samples from the loaded app-owned bundle. Blueprint owns the reusable family templates and generic fallback behavior; the app owns the token values, primitive names, state sets, notes, and implementation hints that feed those templates. The Screens board renders each structured screen in either a `mobile` phone frame or a `desktop` browser frame from `manifest.framePresets`, then fills that frame with lightweight section composition driven by `screens.json` dependencies and bindings. This is prototype evidence, not a full app runtime and not the rejected dashboard-style section metadata-card projection; sidecar files, query helpers, extraction packets, and review metadata remain the source of truth for agents. Every visible token group, primitive, primitive state set, screen frame, and rendered screen section carries `data-boundary-id`/`data-boundary-kind` metadata that is checked against the loaded structured bundle so hand-authored canvas drift fails smoke instead of becoming a hidden handoff bug.
