...
While an MCP-owned `serve` listener is open, Blueprint now watches that sidecar and automatically refreshes the review page after a valid JSON, HTML, CSS, or controlled-asset change. Invalid intermediate edits keep the last-good canvas visible and show a waiting state; once the files become valid, the browser refreshes itself without a manual reload. The same listener exposes a private loopback activity channel. A random per-listener token is written to an owner-only descriptor under the operating-system temporary directory and removed when the MCP connection closes; the token is never included in the served page.

Codex hooks provide the activity input. This repository includes `.codex/hooks.json` for `PreToolUse` and `PostToolUse`, matching Blueprint MCP calls plus `apply_patch` and `Bash`. Codex invokes `src/hooks/codex-activity-hook.ts`, which discovers the listener for the exact project and forwards a bounded event. The browser then switches to the relevant board, fits the inferred boundary, and draws a blue focus frame while the tool is active. Exact MCP boundary arguments produce exact focus; section activity falls back to its containing screen when the section lives inside the isolated prototype iframe, and an unresolvable shell command uses a canvas-wide focus treatment.
