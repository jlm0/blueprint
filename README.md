...
Design findings from the strict lints appear on the canvas as well: literal colors, lengths that restate a token, hand-built native controls, and unmarked sections. A screen frame with findings shows a count beside its name, and every component instance with findings gets a dotted marker inside the frames that render it. The count opens a list of each finding with its source location and remedy, and Copy puts it on the clipboard as an instruction the person can paste to the agent. The list refreshes as fixes land.
